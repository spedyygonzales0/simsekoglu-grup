import { currencyTl, serviceTypeLabel, vehicleCategoryLabel } from "@/lib/i18n";
...
export function buildQuoteMessage(params: QuoteMessageParams): string {
  ...
  if (serviceType === "fleet" && vehicle && variant) {
    const primary = vehicleCategoryLabel(vehicle.primaryCategory, locale);
    const secondary = vehicle.secondaryCategories
      .map((item) => vehicleCategoryLabel(item, locale))
      .join(" / ");
    const categoryLine = secondary ? `${primary} / ${secondary}` : primary;
    ...
    return [
      ...
      `Versiyon: ${variant.title}`,
      `Model Yılı: ${variant.modelYear}`,
